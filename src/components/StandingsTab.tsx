      ...
      <table className="w-full text-sm whitespace-nowrap">
        ...
        <tbody>
          {standings.map((s, i) => {
            ...
            if (!team) return null
            return (
              <tr key={s.team_id} className="border-b border-[#111118] hover:bg-[#111118]/50 transition-colors">
                <td className="p-3">
                  <span className={`inline-flex items-center justify-center w-6 h-6 rounded text-[11px] font-bold ${
                    ...
                    i < 4 ? 'bg-yellow-600/10 text-yellow-400' :
                    'text-slate-500'
                  }`}>{i + 1}</span>
                </td>
                <td className="p-3">
                ...
                <td className="p-3 text-center text-yellow-400">{s.won}</td>
                <td className="p-3 text-center text-slate-400">{s.drawn}</td>
                ...
                <td className={`p-3 text-center font-mono text-sm ${
                  s.goal_difference > 0 ? 'text-yellow-400' : s.goal_difference < 0 ? 'text-red-400' : 'text-slate-500'
                }`}>
                  ...
                  <div className="flex gap-0.5 justify-center">
                    {s.form.map((r, j) => (
                      <span key={j} className={`w-4 h-4 flex items-center justify-center text-[9px] font-bold rounded-sm ${
                        r === 'W' ? 'bg-yellow-600/30 text-yellow-400' :
                        r === 'D' ? 'bg-slate-600/30 text-slate-400' :
