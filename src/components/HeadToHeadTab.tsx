...
export default function HeadToHeadTab({ league, teams, matches }: Props) {
  ...
  const teamObjA = teamAMap.get(teamA)
  const teamObjB = teamAMap.get(teamB)
    ...
    <div>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 mb-8">
        <div className="flex-1 w-full">
          <label className="block text-sm text-slate-400 mb-1">Tim A</label>
          <select
            value={teamA}
            onChange={e => { setTeamA(e.target.value); if (e.target.value === teamB) setTeamB('') }}
            className="w-full px-4 py-3 rounded-xl bg-slate-700 border border-slate-600 text-white focus:border-emerald-500 outline-none text-lg"
          >
        ...
        <div className="flex-1 w-full">
          <label className="block text-sm text-slate-400 mb-1">Tim B</label>
          <select
            value={teamB}
      ...
      {!teamA || !teamB ? (
        <div className="text-center py-16 text-slate-500">
          <Swords className="w-16 h-16 mx-auto mb-4 text-slate-700" />
          <p>Pilih dua tim untuk melihat head to head</p>
        </div>
      ) : (
        <div>
          {/* Summary */}
          {h2hStats && h2hStats.total > 0 ? (
            <div className="p-6 rounded-xl bg-slate-800 border border-slate-700 mb-6">
              <h3 className="text-center text-slate-400 text-sm mb-6">
                {h2hStats.total} pertemuan
              </h3>
              <div className="flex items-center justify-between">
                {/* Team A */}
                <div className="flex-1 text-center">
                  <div className="w-12 h-12 rounded-full mx-auto mb-2" style={{ backgroundColor: teamObjA?.color || '#666' }} />
                  <p className="font-semibold text-white text-lg">{teamObjA?.short_name || teamObjA?.name}</p>
                  <p className="text-3xl font-bold text-white mt-2">{h2hStats.goalsA}</p>
                  <p className="text-xs text-slate-400">Gol</p>
                  <p className="text-lg font-bold text-emerald-400 mt-1">{h2hStats.wonA}W</p>
                </div>
                ...
                <div className="px-6 text-center">
                  ...
                  <p className="text-slate-500 text-sm mt-2">
                    {h2hStats.wonA} - {h2hStats.drawn} - {h2hStats.wonB}
                  </p>
                ...
                <div className="flex-1 text-center">
                  ...
                  <p className="font-semibold text-white text-lg">{teamObjB?.short_name || teamObjB?.name}</p>
                  <p className="text-3xl font-bold text-white mt-2">{h2hStats.goalsB}</p>
                  <p className="text-xs text-slate-400">Gol</p>
                  <p className="text-lg font-bold text-emerald-400 mt-1">{h2hStats.wonB}W</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="p-6 rounded-xl bg-slate-800 border border-slate-700 mb-6 text-center">
              <p className="text-slate-400">Belum ada pertemuan</p>
            </div>
          )}
          ...
          {h2hMatches.length === 0 ? (
            ...
            <div className="space-y-2">
              {h2hMatches.map(m => {
                const home = teamAMap.get(m.home_team_id)
                const away = teamAMap.get(m.away_team_id)
                const isPlayed = m.status === 'played'
                const isSkipped = m.status === 'skipped'
                return (
                  <div key={m.id} className="flex items-center justify-between p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                    <div className="flex items-center gap-2 flex-1">
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: home?.color || '#666' }} />
                      <span className={`text-sm font-medium ${isPlayed && m.home_score != null && m.away_score != null && m.home_score > m.away_score ? 'text-emerald-400' : 'text-white'}`}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isPlayed ? (
                        ...
                        <span className="text-sm italic text-slate-500">Skip</span>
                      ) : (
                    ...
                    <div className="flex items-center gap-2 flex-1 justify-end">
                      <span className={`text-sm font-medium ${isPlayed && m.away_score != null && m.home_score != null && m.away_score > m.home_score ? 'text-emerald-400' : 'text-white'}`}>
