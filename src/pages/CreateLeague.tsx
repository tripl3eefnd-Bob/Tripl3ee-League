...
import type { League, CompetitionType, LeagueFormat, Tiebreaker, LeagueCategory } from '../types'
...
export default function CreateLeague() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const league = await createLeague({
      ...
      competition_type: form.competition_type, category: form.category,
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
        <div className="p-4 rounded-lg bg-[#111118] space-y-4 border border-[#1e1e2a]">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select value={form.category}
                onChange={e => setForm(p => ({ ...p, category: e.target.value as LeagueCategory }))}
                className="w-full px-3 py-2 rounded-lg bg-[#0a0a0f] border border-[#1e1e2a] text-white text-sm focus:border-yellow-500 outline-none">
              ...
              <button key={type} type="button" onClick={() => setForm(p => ({ ...p, competition_type: type }))}
                className={`p-2.5 rounded-lg border text-xs font-medium transition-colors ${
                  form.competition_type === type
                    ...
                    : 'bg-[#0a0a0f] border-[#1e1e2a] text-slate-400 hover:border-slate-500'
                }`}>
              ...
              <button key={tb} type="button" onClick={() => toggleTiebreaker(tb)}
                className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${
                  form.tiebreaker.includes(tb)
                    ? 'bg-yellow-600/20 border-yellow-500 text-yellow-400'
                    : 'bg-[#0a0a0f] border-[#1e1e2a] text-slate-400'
                }`}>
        ...
        <button type="submit" disabled={saving || !form.name}
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-yellow-600 text-white text-sm font-semibold hover:bg-yellow-500 disabled:opacity-50">
